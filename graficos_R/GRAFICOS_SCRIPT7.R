...
# Filtro
# -------------
Hora_cp2 <- est$Hora #[-c(which(est$Dia=="SABADO"),which(est$Dia=="DOMINGO"))]
...
# -------- #
grafs <- paste("JPG/horario_bar_total_v1.jpg",sep="")
jpeg(grafs,width = 25,height=10,pointsize=11,res = 200,units = "cm")
par(mar=c(10.1,8.1,5.1,1.1),mfrow=c(1,1),mex=0.65)

colfunc <- c("#8B3E2F",brewer.pal(11,'RdYlBu'))

barplot(as.matrix(TOTAL), main="Mortes de Ciclistas por horário",yaxt="n",
        cex.names=0.7, beside=TRUE, col=colfunc,font=2,las=2) 
mtext(side = 2, line =  4, "Número de mortes")
axis(side = 2,at = 0:9,las=2,cex.lab=3)
